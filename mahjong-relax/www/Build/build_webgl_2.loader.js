function createUnityInstance(e, t, r) {
	...
	c.SystemInfo = function() {
		...
		return a(e, "", 0), !0
	}, Error.stackTraceLimit = Math.max(Error.stackTraceLimit || 0, 50), c.readBodyWithProgress = function() {
		function e(e, t) {
			if (!t) return 0;
		...
		function t(t, r) {
			function n() {
				return "undefined" == typeof i ? t.arrayBuffer().then(function(e) {
					...
					return e.done ? o() : (f + e.value.length <= l.length ? (l.set(e.value, f), u = f + e.value.length) : d.push(e.value), f += e.value.length, r({
						type: "progress",
						total: Math.max(s, f),
						loaded: f,
						lengthComputable: a
			...
			var i = t.body ? t.body.getReader() : void 0,
				...
				f = 0,
				u = 0;
			return a || console.warn("[UnityCache] Response is served without Content-Length header. Please reconfigure server to include valid Content-Length for better download performance."), n().then(function(e) {
				return r({
					type: "load",
					total: e.length,
					loaded: e.length,
			...
			require: function(e) {
				var t = {
					...
					"zlib/inflate.js": function(e, t, r) {
						...
						function i(e) {
							var t;
							return e && e.state ? (t = e.state, e.total_in = e.total_out = t.total = 0, e.msg = "", t.wrap && (e.adler = 1 & t.wrap), t.mode = F, t.last = 0, t.havedict = 0, t.dmax = 32768, t.head = null, t.hold = 0, t.bits = 0, t.lencode = t.lendyn = new p.Buf32(ge), t.distcode = t.distdyn = new p.Buf32(me), t.sane = 1, t.back = -1, T) : A
						}
						...
						function c(e, t) {
							var r, o, i, a, s, l, d, c, h, b, g, m, w, ge, me, we, pe, ve, ke, ye, _e, xe, Se, Ee, Ue = 0,
								Ce = new p.Buf8(4),
							...
							e: for (;;) switch (r.mode) {
								case F:
									...
									if (2 & r.wrap && 35615 === c) {
										r.check = 0, Ce[0] = 255 & c, Ce[1] = c >>> 8 & 255, r.check = k(r.check, Ce, 2, 0), c = 0, h = 0, r.mode = N;
										break
									}
									...
									r.dmax = 1 << _e, e.adler = r.check = 1, r.mode = 512 & c ? V : Y, c = 0, h = 0;
									break;
								case N:
									for (; h < 16;) {
										if (0 === l) break e;
									...
									r.head && (r.head.text = c >> 8 & 1), 512 & r.flags && (Ce[0] = 255 & c, Ce[1] = c >>> 8 & 255, r.check = k(r.check, Ce, 2, 0)), c = 0, h = 0, r.mode = Z;
								case Z:
									for (; h < 32;) {
										if (0 === l) break e;
										l--, c += o[a++] << h, h += 8
									}
									r.head && (r.head.time = c), 512 & r.flags && (Ce[0] = 255 & c, Ce[1] = c >>> 8 & 255, Ce[2] = c >>> 16 & 255, Ce[3] = c >>> 24 & 255, r.check = k(r.check, Ce, 4, 0)), c = 0, h = 0, r.mode = H;
								case H:
									for (; h < 16;) {
										if (0 === l) break e;
										l--, c += o[a++] << h, h += 8
									}
									r.head && (r.head.xflags = 255 & c, r.head.os = c >> 8), 512 & r.flags && (Ce[0] = 255 & c, Ce[1] = c >>> 8 & 255, r.check = k(r.check, Ce, 2, 0)), c = 0, h = 0, r.mode = D;
								case D:
									if (1024 & r.flags) {
										for (; h < 16;) {
											if (0 === l) break e;
										...
										r.length = c, r.head && (r.head.extra_len = c), 512 & r.flags && (Ce[0] = 255 & c, Ce[1] = c >>> 8 & 255, r.check = k(r.check, Ce, 2, 0)), c = 0, h = 0
								...
								case te:
									for (; r.have < r.nlen + r.ndist;) {
										for (; Ue = r.lencode[c & (1 << r.lenbits) - 1], me = Ue >>> 24, we = Ue >>> 16 & 255, pe = 65535 & Ue, !(me <= h);) {
											if (0 === l) break e;
											l--, c += o[a++] << h, h += 8
										}
								...
								case ne:
									...
									for (r.back = 0; Ue = r.lencode[c & (1 << r.lenbits) - 1], me = Ue >>> 24, we = Ue >>> 16 & 255, pe = 65535 & Ue, !(me <= h);) {
										if (0 === l) break e;
										l--, c += o[a++] << h, h += 8
									}
									if (we && 0 === (240 & we)) {
										for (ve = me, ke = we, ye = pe; Ue = r.lencode[ye + ((c & (1 << ve + ke) - 1) >> ve)], me = Ue >>> 24, we = Ue >>> 16 & 255, pe = 65535 & Ue, !(ve + me <= h);) {
											if (0 === l) break e;
											l--, c += o[a++] << h, h += 8
										}
										c >>>= ve, h -= ve, r.back += ve
								...
								case ie:
									for (; Ue = r.distcode[c & (1 << r.distbits) - 1], me = Ue >>> 24, we = Ue >>> 16 & 255, pe = 65535 & Ue, !(me <= h);) {
										if (0 === l) break e;
										l--, c += o[a++] << h, h += 8
									}
									if (0 === (240 & we)) {
										for (ve = me, ke = we, ye = pe; Ue = r.distcode[ye + ((c & (1 << ve + ke) - 1) >> ve)], me = Ue >>> 24, we = Ue >>> 16 & 255, pe = 65535 & Ue, !(ve + me <= h);) {
											if (0 === l) break e;
											l--, c += o[a++] << h, h += 8
										}
										c >>>= ve, h -= ve, r.back += ve
							...
							return e.next_out = s, e.avail_out = d, e.next_in = a, e.avail_in = l, r.hold = c, r.bits = h, (r.wsize || g !== e.avail_out && r.mode < ce && (r.mode < de || t !== U)) && u(e, e.output, e.next_out, g - e.avail_out) ? (r.mode = he, I) : (b -= e.avail_in, g -= e.avail_out, e.total_in += b, e.total_out += g, r.total += g, r.wrap && g && (e.adler = r.check = r.flags ? k(r.check, i, g, e.next_out - g) : v(r.check, i, g, e.next_out - g)), e.data_type = r.bits + (r.last ? 64 : 0) + (r.mode === Y ? 128 : 0) + (r.mode === re || r.mode === X ? 256 : 0), (0 === b && 0 === g || t === U) && xe === T && (xe = z), xe)
						}
						...
						function g(e, t) {
							...
							return e && e.state ? (r = e.state, 0 !== r.wrap && r.mode !== K ? A : r.mode === K && (n = 1, n = v(n, t, i, 0), n !== r.check) ? O : (o = u(e, t, i, i)) ? (r.mode = he, I) : (r.havedict = 1, T)) : A
						}
					...
					"zlib/inffast.js": function(e, t, r) {
						...
						t.exports = function(e, t) {
							var r, i, a, s, l, d, f, u, c, h, b, g, m, w, p, v, k, y, _, x, S, E, U, C, B;
							r = e.state, i = e.next_in, C = e.input, a = i + (e.avail_in - 5), s = e.next_out, B = e.output, l = s - (t - e.avail_out), d = s + (e.avail_out - 257), f = r.dmax, u = r.wsize, c = r.whave, h = r.wnext, b = r.window, g = r.hold, m = r.bits, w = r.lencode, p = r.distcode, v = (1 << r.lenbits) - 1, k = (1 << r.distbits) - 1;
							e: do {
								m < 15 && (g += C[i++] << m, m += 8, g += C[i++] << m, m += 8), y = w[g & v];
								t: for (;;) {
									if (_ = y >>> 24, g >>>= _, m -= _, _ = y >>> 16 & 255, 0 === _) B[s++] = 65535 & y;
										...
										x = 65535 & y, _ &= 15, _ && (m < _ && (g += C[i++] << m, m += 8), x += g & (1 << _) - 1, g >>>= _, m -= _), m < 15 && (g += C[i++] << m, m += 8, g += C[i++] << m, m += 8), y = p[g & k];
										r: for (;;) {
											...
											if (S = 65535 & y, _ &= 15, m < _ && (g += C[i++] << m, m += 8, m < _ && (g += C[i++] << m, m += 8)), S += g & (1 << _) - 1, S > f) {
											...
											if (g >>>= _, m -= _, _ = s - l, S > _) {
												...
												for (; x > 2;) B[s++] = U[E++], B[s++] = U[E++], B[s++] = U[E++], x -= 3;
					...
					"zlib/inftrees.js": function(e, t, r) {
						...
						t.exports = function(e, t, r, b, g, m, w, p) {
							var v, k, y, _, x, S, E, U, C, B = p.bits,
								T = 0,
								L = 0,
								R = 0,
								A = 0,
							...
							if (e === s ? (Z = M = w, S = 19) : e === l ? (Z = f, H -= 257, M = u, W -= 257, S = 256) : (Z = c, M = h, S = -1), N = 0, L = 0, T = R, x = m, I = O, z = 0, y = -1, F = 1 << O, _ = F - 1, e === l && F > i || e === d && F > a) return 1;
							for (;;) {
								E = T - z, w[L] < S ? (U = 0, C = w[L]) : w[L] > S ? (U = M[W + w[L]], C = Z[H + w[L]]) : (U = 96, C = 0), v = 1 << T - z, k = 1 << I, R = k;
								do k -= v, g[x + (N >> z) + k] = E << 24 | U << 16 | C | 0; while (0 !== k);
								for (v = 1 << T - 1; N & v;) v >>= 1;
								...
								if (T > O && (N & _) !== y) {
									for (0 === z && (z = O), x += R, I = T - z, P = 1 << I; I + z < A && (P -= D[I + z], !(P <= 0));) I++, P <<= 1;
	...
	return new Promise(function(e, t) {
		if (c.SystemInfo.hasWebGL)
			if (1 == c.SystemInfo.hasWebGL) {
				var n = 'Your browser does not support graphics API "WebGL 2" which is required for this content.';
				"Safari" == c.SystemInfo.browser && parseInt(c.SystemInfo.browserVersion) < 15 && (n += c.SystemInfo.mobile || navigator.maxTouchPoints > 1 ? "\nUpgrade to iOS 15 or later." : "\nUpgrade to Safari 15 or later."), t(n)
			} else c.SystemInfo.hasWasm ? (c.startupErrorHandler = t, r(0), c.postRun.push(function() {
